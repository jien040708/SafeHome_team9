...
- Result: 312 tests, **76%** overall coverage (7,364 statements / 1,778 misses)
- Largest gaps: `main.py` (23%), `ui/main_window.py` (12%), `event_logging/log.py` (25%), `virtual_device_v4/device/safehome_sensor_test.py` (28%)
...
| Phase 3 - Logging & Notifier Hardening | Lift `event_logging/log_manager.py` to 70% and `devices/siren.py` to 95% | `event_logging/log_manager.py`, `devices/siren.py`, `event_logging/log.py` | Complete: storage is injectable with retry-aware tests plus siren idempotence coverage; next focus on `event_logging/log.py` retries. | Infra |
| Phase 4 ??Virtual Device Parity | Increase `virtual_device_v4/device/safehome_sensor_test.py` to 60% and `interface_camera.py` to 85% | `virtual_device_v4/device/safehome_sensor_test.py`, `virtual_device_v4/device/interface_camera.py` | Break monolithic script into functions; add pytest-driven simulation tests verifying camera/mock sensor behavior. | Surveillance |
...
3. **Logging Reliability**
   - [x] Build in-memory/DI-friendly fixture to test `event_logging/log_manager.py` insert/filter logic.
   - [ ] Simulate DB failure (raise `sqlite3.Error`) to assert retry/error logging behavior.
   - [x] Cover `devices/siren.py` activation/deactivation transitions including duplicate calls.
   - [x] Simulate DB failure (raise `sqlite3.Error`) to assert retry/error logging behavior.
   - [x] Split `virtual_device_v4/device/safehome_sensor_test.py` into importable helpers; add tests for windoor/motion pathways.
   - [x] Flesh out `interface_camera.py` tests ensuring each abstract method raises `NotImplementedError`.
   - [x] Provide placeholder image fixture for camera IDs > existing JPGs (done in code; add regression test).
